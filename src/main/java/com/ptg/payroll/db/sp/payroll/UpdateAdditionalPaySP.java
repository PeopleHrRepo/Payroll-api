...
public class UpdateAdditionalPaySP extends StoredProcedure {

    private static final Logger log = LoggerFactory.getLogger(UpdateAdditionalPaySP.class);

    private static final String SP_NAME = "TRINET_PAYROLL_ENTRY.spc_InitializeEEAddlPayData";
    ...
    public UpdateAdditionalPaySP(DataSource dataSource) {
        ...
        declareParameter(new SqlParameter(IN_EMPLID, Types.VARCHAR));
    ...
    public void execute(PayGroup payGroup, String emplid) {
        ...
        if (isError) {
            log.error("SQL Error:" + error + " Unable to update Additional Pay Data UpdateAdditionalPaySP: sp {}, parameters:{}", SP_NAME, inputs);
            throw new GatewayBackendException("SQL Error:" + " UpdateAdditionalPaySP not able to update Additional Pay:");
        }
